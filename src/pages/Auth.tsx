...
export default function Auth() {
  ...
  const handleGoogleLogin = async () => {
    ...
    if (result?.error) toast({ title: "Erreur Google", description: String(result.error), variant: "destructive" });
    setLoading(false);
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      <Card className="w-full max-w-md border-border/50">
        <CardHeader className="text-center space-y-3">
          ...
          <CardDescription className="text-muted-foreground">Gérez vos événements comme un pro</CardDescription>
        </CardHeader>
          ...
          <Tabs defaultValue="login" className="w-full">
            ...
            <TabsContent value="login">
              <form onSubmit={handleLogin} className="space-y-3 mt-3">
                ...
                <div className="space-y-1.5"><Label>Mot de passe</Label><div className="relative"><Lock className="absolute left-3 top-3 h-4 w-4 text-muted-foreground"/><Input type="password" placeholder="••••••••" className="pl-9" value={password} onChange={e=>setPassword(e.target.value)} required/></div></div>
